...
namespace core::lexer {
    ...
    Lexer::Lexer(const std::string_view source) : source(source), categories{} {
        for (auto& entry : categories) {
            entry = Type::OTHER;
        }

        for (int i = 'a'; i <= 'z'; ++i) { categories[i] = Type::LETTER; }
        for (int i = 'A'; i <= 'Z'; ++i) { categories[i] = Type::LETTER; }
    ...
    Token Lexer::advance() {
        ...
        if (state == Type::ESCAPE) {
            ...
            this->column++;
            if (this->end < this->source.length()) {
                if (categories[static_cast<unsigned char>(this->source[this->end])] == Type::LETTER) {
                    while (this->end < this->source.length() && categories[static_cast<unsigned char>(this->source[this->end])] == Type::LETTER) {
                        this->end++;
                        this->column++;
                    }
        ...
        if (state != Type::OTHER && state != Type::LETTER) {
            this->end++;
            this->column++;
